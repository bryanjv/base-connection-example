...
public class MainActivity extends AppCompatActivity implements MainView {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Retrofit restAdapter = new Retrofit.Builder().baseUrl("http://www.mocky.io/").addConverterFactory(GsonConverterFactory.create()).build();

        RetrofitConnection service = restAdapter.create(RetrofitConnection.class);
        Call<List<Libro>> biblio = service.getLibro();

        biblio.enqueue(new Callback<List<Libro>>() {
            @Override
